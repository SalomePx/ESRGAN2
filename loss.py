...
def ContentLoss(criterion='l1', output_layer=54, before_act=True):
    ...
    def content_loss(hr, sr):
        ...
        sr_3d = tf.concat([sr, sr, sr], axis=-1)
        hr_3d = tf.concat([hr, hr, hr], axis=-1)
        preprocess_sr = preprocess_input(sr_3d * 255.) / 12.75
        preprocess_hr = preprocess_input(hr_3d * 255.) / 12.75
